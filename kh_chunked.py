...
# Full KH amplitude summed over intermediate states
A_total = np.einsum(
    'afn,mbn->mfab',
    mu_fn,
    mu_ni_weighted,
...
A_total_per_f = A_total

# Compute emission weighting
second_term = gamma_f / (((Ef_ - Ei_ - E_ex_ + E_em_)**2) + 0.25*gamma_f**2)
second_term_sum_em = np.sum(second_term, axis=2)  # (M, N_f)

# Allocate decomposed contributions
chunk_size = 200

I_no_int_per_n = np.zeros((M, N_n), dtype=np.float64)
C_full_per_n = np.zeros((M, N_n), dtype=np.float64)

for start in range(0, N_n, chunk_size):
    end = min(start + chunk_size, N_n)
    sl = slice(start, end)

    # a_n chunk: <f|D|n><n|D|i> / denominator
    a_chunk = oe.contract(
        'rfn,mln->mfrln',
        mu_fn[:, :, sl],
        mu_ni_weighted[:, :, sl]
    ...
    I_chunk = np.abs(a_chunk)**2
    I_chunk = np.sum(I_chunk, axis=(2, 3))  # (M, N_f, chunk)

    I_no_int_per_n[:, sl] = np.einsum(
        'mfn,mf->mn',
        I_chunk,
        second_term_sum_em,
        optimize=True
    ...
    # Full interference contribution
    C_chunk = np.real(
        np.einsum(
            'mfrln,mfrl->mfn',
            a_chunk,
    ...
    print(f"Processed intermediate states {start} → {end}")
...
C_full_per_f = np.sum(np.abs(A_total_per_f)**2, axis=(2, 3)) * second_term_sum_em
total_by_m_f = np.sum(C_full_per_f, axis=1)
